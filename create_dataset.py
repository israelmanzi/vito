...
try:
    connetion = sqlite3.connect('faces.db')
...
if len(faces) > 0:
    ...
    while True:
        ...
        for (x, y, w, h) in faces:
            ...
            # Check if enough time has passed to capture an image and if image count is less than 250
            if (time.time() - current_time) * 1000 >= interval and image_count < 250:
        ...
        # To stop taking video, press 'q' key or if image count reaches 250
        if cv2.waitKey(1) & 0xFF == ord('q') or image_count >= 250:
            break
